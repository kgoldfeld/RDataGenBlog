...
author: ''
date: '2019-03-12'
slug: a-case-where-prospecitve-matching-may-limit-bias
categories: []
tags:
  - R
subtitle: ''
...
dused <- rbind(dused, dsamp)
dselect1 <- rbind(dr, dc)

setnames(dr, c("ID", "age", "BMI"), c("ID1", "a1","b1"))
setnames(dc, c("ID", "age", "BMI"), c("ID2", "a2","b2"))
dc[, `:=`(female = NULL, rx = NULL)]
dselect2 <- cbind(dr, dc)

ggplot(data = dselect1, aes(x = age, y = BMI)) +
  geom_point(pch = 20, color = "blue") +
  geom_segment(data = dselect2, aes(x=a1, y=b1, xend = a2, yend = b2),
               color = "blue") +
  geom_point(data = dused, pch = 19, size = 1, color = "red") +
  facet_grid(~factor(female, labels = c("male", "female"))) +
...
```{r, echo = FALSE}
dmale <- rbind(drand[female == 0], dcntl[female == 0])
dmale[, .(.N, mu.age = mean(age), sd.age = sd(age), 
              mu.bmi = mean(BMI), sd.bmi = sd(BMI)), keyby = rx]
```
...
```{r, echo = FALSE}
dfemale <- rbind(drand[female == 1], dcntl[female == 1])
dfemale[, .(.N, mu.age = mean(age), sd.age = sd(age), 
                mu.bmi = mean(BMI), sd.bmi = sd(BMI)), keyby = rx]
...
<p><small><font color="darkkhaki">This study is supported by the National Institutes of Health National Institute of Diabetes and Digestive and Kidney Diseases R01DK11048. The views expressed are those of the author and do not necessarily represent the official position of the funding organizations.</font></small></p>
