...
The data generation process parallels the earlier [post](https://www.rdatagen.net/post/a-case-where-prospecitve-matching-may-limit-bias/). The treatment assignment is made in the context of the matching process, which I am not showing this time around. Note that in this initial example, the outcome `y` depends on the intervention `rx` (i.e. there *is* an intervention effect).
...
```{r, message = FALSE, fig.width = 6, fig.height = 5, echo = FALSE}
library(ggmosaic)
...
ggplot(data = pdata) +
  ...
  geom_label(data = pdata, 
          ...
          size = 3.2) +
  scale_x_continuous(position = "top", breaks = textloc$x, 
                     labels = c("no success", "success"), 
                     name = "intervention",
                     expand = c(0,0)) +
...
```{r, echo = FALSE}
load("DataPaired/resnoeffect.Rdata")
```
...
Here are the summary statistics for a data set with an intervention effect of 0.45. The results are consistent with the "no effect" simulations, except that the standard linear regression model exhibits some bias. In reality, this is not necessarily bias, but a different estimand. The model that ignores clustering is a marginal model (with respect to the site), whereas the conditional logistic regression and mixed effects models are conditional on the site. (I've described this phenomenon [here](https://www.rdatagen.net/post/marginal-v-conditional/) and [here](https://www.rdatagen.net/post/mixed-effect-models-vs-gee/).) We are interested in the conditional effect here, so that argues for the conditional models.
...
```{r, echo = FALSE}
load("DataPaired/reseffect.Rdata")
...
bias <- data.frame(
  clr = mean(sapply(res, function(x) x$clr["rx", "coef"])) - 0.45,
  glm = mean(sapply(res, function(x) x$glm["rx", "Estimate"])) - 0.45,
  glmer = mean(sapply(res, function(x) x$glmer["rx", "Estimate"])) - 0.45
)
...
```{r, echo = FALSE}
load("DataPaired/reseffect_noclust.Rdata")
...
<p><small><font color="darkkhaki">The DREAM Initiative is supported by the National Institutes of Health National Institute of Diabetes and Digestive and Kidney Diseases R01DK11048. The views expressed are those of the author and do not necessarily represent the official position of the funding organizations.</font></small></p>
