---
title: Design effects with baseline measurements
...
Two colleagues on the [Design and Statistics Core](https://impactcollaboratory.org/design-and-statistics-core/){target="_blank"} of the [NIA IMPACT Collaboratory](https://impactcollaboratory.org/){target="_blank"}, Monica Taljaard and Fan Li, and I have started to think about baseline outcome information in the context of a stepped wedge cluster randomized trial. The question we are focused on is *if we are able to measure an outcome at both baseline and follow-up for each individual in the study, how much can we reduce the number clusters or sites needed for the study*? Ultimately, we hope to have an analytic solution for this, but to get things started, we thought we could explore using simulation. I'll be sharing some of those results in this and upcoming posts.

Before we get to the case of the stepped wedge design, I thought it might be useful to start with the simpler case of a parallel cluster randomized design. I have previously written generally about the [design effect](https://www.rdatagen.net/post/what-exactly-is-the-design-effect/){target="_blank"}, but a [paper](https://onlinelibrary.wiley.com/doi/full/10.1002/sim.5352){target="_blank"} titled "A simple sample size formula for analysis of covariance in cluster randomized trials" by *Teerenstra et al* provides a great jumping off point to understand how baseline measurements can impact sample sizes. 

In this initial exploration, I am simulating data from a series of study designs to illustrate the impact of the design effect on sample size requirements, focusing on the number of clusters. The repeated measurement design effects were all drawn from the *Teerenstra et al* paper.

## Quick overview

This post is a bit longer than usual, but there was no obvious way to split it apart; besides, once you get the hang of it, it flows pretty smoothly (says the blogger). After showing an example based on a simple 2-arm randomized control trial with 350 subjects that has 80% power to detect a standardized effect size of 0.3, I describe a series of cluster randomized designs with cluster sizes of 30 subjects that require progressively fewer clusters to achieve 80% under the same effect size and total variance assumptions: a simple cluster randomized needs 64 sites, a cross-sectional pre-post design needs 52, a repeated measures design needs 38, and a repeated measures design that models follow-up outcomes only (i.e. and ANCOVA model) requires only 32.
...
If we need to randomize at the site level (i.e., conduct a cluster randomized trial), we can describe the data generation process as
...
The parameters for this simulation are $\delta_1 = 2.4$, $\sigma_c^2 = 6.8$, $\sigma_{cp}^2 = 2.8$, $\sigma_{sp}^2 = 54.4$.
...
The design effect for the cluster randomized trial without any baseline measurement was 5.35. With the two-cohort design, the design effect is reduced slightly:
...
Based on the variance assumptions, we can update our design effect a third time:
...
Once again, the design effect (and sample size requirement) is reduced:
