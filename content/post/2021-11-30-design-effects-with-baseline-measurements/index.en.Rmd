...
Is it possible to reduce the size requirements of a stepped wedge cluster randomized trial simply by collecting baseline information? It is generally the case that if we are able to measure an outcome for a subject at two time periods, first at baseline and then at follow-up, we can reduce sample size. Does this extend to the context of cluster randomized trials generally, and stepped wedge designs more specifically? The answer to the first part of the question is a definite "yes," as described in a 2012 paper *Teerenstra et al* (some more details on that below). 

As for the second part of the question, two colleagues on the [Design and Statistics Core](https://impactcollaboratory.org/design-and-statistics-core/){target="_blank"} of the [NIA IMPACT Collaboratory](https://impactcollaboratory.org/){target="_blank"}, Monica Taljaard and Fan Li, and I have just started thinking about this. Ultimately, we hope to have an analytic solution that provides more formal guidance for stepped wedge designs, but to get things started, we thought we could explore a bit using simulation.
...
It turns out that if we collect baseline QOL scores and can "control" for those measurements in some way (by conducting a repeated measures analysis, using ANCOVA, or assessing the difference itself as an outcome), we might be able to reduce the variability across study subjects sufficiently to give us a better chance at picking up the signal. I've [written](https://www.rdatagen.net/post/thinking-about-the-run-of-the-mill-pre-post-analysis/
){target="_blank"} about this concept before in the context of clinical trials where randomization is at the subject level; now we will turn to the case where randomization is at the cluster or site level.

This post will focus on work already done to derive *design effects* for parallel cluster randomized trials that collect baseline measurements. (We will get to stepped wedge designs in future posts.) I described the [design effect](https://www.rdatagen.net/post/what-exactly-is-the-design-effect/){target="_blank"} more generally in an earlier post, but the [paper](https://onlinelibrary.wiley.com/doi/full/10.1002/sim.5352){target="_blank"} by *Teerenstra et al*, titled "A simple sample size formula for analysis of covariance in cluster randomized trials" provides a better jumping off point to understand how baseline measurements can impact sample sizes in clustered designs. In this initial exploration, I am simulating data from a series of study designs to illustrate the impact of the design effect on sample size requirements.

Here's a brief outline of what follows: after showing an example based on a simple 2-arm randomized control trial with 350 subjects that has 80% power to detect a standardized effect size of 0.3, I describe a series of designs with cluster sizes of 30 subjects that require progressively fewer clusters to achieve 80% under the same effect size and total variance assumptions: a simple cluster randomized that needs 64 sites, a cross-sectional pre-post design that needs 52, a repeated measures design needs that 38, and a repeated measures design that models follow-up outcomes only (i.e. uses an ANCOVA model) that requires only 32.
