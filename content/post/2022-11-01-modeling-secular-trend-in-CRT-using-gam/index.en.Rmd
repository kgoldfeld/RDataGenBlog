...
The `simstudy` definitions prepare for $b^0$, $A$, $D$, and $Y$. The vector $\mathbf{b^1}$ is generated separately in the actual data generation process using `addCorGen`, using $\mu = 0$ and $\sigma^2 = 16$. Here are the definitions:
...
And here is the data generation process. `addCorGen` adds the correlated cluster-time effects $\mathbf{b^1}$:
...
where we have a single parameter $\gamma$ instead of $k$ parameters. Here is an estimate of the treatment effect $\delta$ using a mixed effects model assuming a common linear time trend:
...
The linear model gets around the problem of a large number parameters, but it imposes a very strong assumption that the outcome $Y$ changes linearly over time. This is unlikely to be the case. We could fit a quadratic model like

$$
Y_{ijk} = \beta_0  + \gamma_0 k + \gamma_1 k^2 + \delta A_j + b_j  + e_{ijk}
$$

but the assumption is still quite strong. We could fit a mixed effects model with a random slope $b_{1j}$ as well:
...
Since there is no reason to believe that each cluster shares the same time trend, as assumed in the first two models estimated here. So now we introduce additional flexibility by fitting random cubic spline for each cluster.
...
The figure below shows the predicted site-specific curves for each of the models estimated here. The rigidity of the linear and fixed cubic spline models is pretty clear here. And in this case, the two more flexible methods appear to generate quite similar predicted curves.
...
The next figure shows the individual-level outcomes and the predicted curves for a small number of sites. It is clear that the curves for the less flexible methods are biased. Also, the similarity of the flexible models is particularly evident here.
...
The results from a single data set are interesting, but we really need to understand how well the models perform over a large number of data sets. How do the model estimates of the true treatment effect $\delta = 5$ compare when considering bias, variance, and 95% coverage?

The replication process requires generating data (in function `s_generate`) and then fitting the models (in function `replicate`). The data generation uses the data definitions provided earlier in the post. In this case, we will use 1000 data sets.
...
Each replication provides the point estimate of the treatment effect as well as the estimate of the standard error. Here is what the results look like:
...
As I think about using the flexible random cubic spline and GAM models to evaulate a stepped wedge design, I am very curious to see how the bias-variance trade-off plays out and to see if the less flexible methods continue to perform as well as the GAM model, at least with respect to the parameter of interest. More to come.
