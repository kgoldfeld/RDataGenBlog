...
In the first example, an individual in a cluster is measured once during a specific time period, but the measurements for a particular cluster can span multiple time periods, so different individuals in the same cluster might be in different time periods. 

If we have 3 time periods and 3 individuals in each time period, the correlation between two individuals in the same time period might be $\alpha_1$, the correlation between individuals in adjacent time periods (periods 1/2 and periods 2/3) is $\alpha_2$, and the correlation between individuals in time periods 1 and 3 would be $\alpha_3$. The correlation structure for the cluster would be represented like this with each period represented in $3 \times 3$ sub-blocks:
...
In the second example, we specify using an auto-regressive structure with $\alpha = 0.3$, so that $\alpha_1 = \alpha =0.3$, $\alpha_2 = \alpha^2 = 0.09$, and $\alpha_3 = \alpha^3 = 0.027$:
...
Using the first target correlation matrix ($\alpha_1 = 0.3$, $\alpha_2 = 0.2$, and $\alpha_3 = 0.1$), we can go ahead and generate some data. The probability of outcome $Y$ will be a function of cluster-level treatment $A$, $A \in \{0,1\}$ and individual-level covariate $z$, a continuous measure centered closely around 0. Here are the data definitions:
...
To convince myself that the data are indeed being generated intended, I have generated a large number of data sets using `addCorGen` while fixing the individual level probabilities. The empirical within-cluster correlation can be estimated as the observed correlation of the repeated data sets (within each cluster). I am outputting the nine $\pi$'s for the individuals in a single cluster next to the mean observed outcome for that individual. And, more importantly, I am showing the correlation calculated based on the 2,500 sets of outcomes. In both cases, the observed values are very much consistent with the true values used to generate the data. Thoth I am not showing you, this is the case for all 10 clusters.
...
In this next example, I will simulate a case where there are two measurement periods but for the same individual in each period (say a baseline and follow-up). In this case, the within-individual (between period) correlation ($\alpha^*$) can be different from the between-individual, within-period correlation ($\alpha_1$) and the between-individual, between-period correlation ($\alpha_2$): 
...
`genBlockMat` implements this with an additional argument *iRho*. Here we are using *iRho* to set $\alpha^* = 0.5$:
...
The data generation process here is slightly modified to accommodate the structure of individuals repeated in in both time periods. The clusters are generated first, as before, but then the individuals are created within the clusters. Once the individuals are created, then the time periods are introduced before generating the outcome (which in this case depends on the time period). Before generating the correlated outcomes using `addCorGen`, it is now important that the data be sorted by cluster, period, *and* individual, as this is in line with the correlation structure.
...
I've alluded to the fact that `genBlockMat` and `addCorGen` cannot accommodate varying cluster sizes (yet), but if we need to generate clustered data with correlated outcomes and varying cluster sizes in a *single* period using `addCorGen`, that *is* now possible. I mentioned that I made some adjustments to `addCorGen` (primarily to speed up its performance), but I was able to make it more flexible as well. Here is a simple example to demonstrate how this works.
