...
In my last [post](https://www.rdatagen.net/post/intra-cluster-correlations-over-time/), I wrote about *within-* and *between-period* intra-cluster correlations in the context of stepped-wedge cluster randomized study designs. These are quite important to understand when figuring out sample size requirements (and models for analysis, which I'll be writing about soon.) Here, I'm extending the constant ICC assumption I presented last time around by introducing some complexity into the correlation structure. Much of the code I am using can be found in last week's post, so if anything seems a little unclear, hop over [here](https://www.rdatagen.net/post/intra-cluster-correlations-over-time/).
...
This paper by [*Kasza et al*](https://journals.sagepub.com/doi/full/10.1177/0962280217734981), which describes various stepped-wedge models, suggests a structured variation of $\mathbf{R}$ that is a function of two parameters, $r_0$ and $r$:
...
![](/img/post-iccvary/p2.png){width=800px}
...
![](/img/post-iccvary/p3.png){width=800px}
...
$$ ITT_{33} = \frac{0.15 + 3^2 \times 0.01}{0.15 + 3^2 \times 0.01 + 2} =0.1071$$ 
and
...
![](/img/post-iccvary/p4.png){width=800px}
