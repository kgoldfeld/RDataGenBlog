...
Over the past few years, a number of folks have asked if `simstudy` accommodates customized distributions. There's been an interest in truncated, zero-inflated, or even more standard distributions that have not yet been implemented in `simstudy`. I've come up with solutions in specific cases, but was never able to come up with anything general enough that might satisfy this need.

Looking through the [issue](https://github.com/kgoldfeld/simstudy/issues){target="_blank"} list last week, I had a brief moment of inspiration, so I decided to tackle the problem anew. And I am happy to say that custom distributions can now be specified in `defData` and `defDataAdd` by setting the argument *dist* to "custom". While not perfect, the new functionality has the potential to be quite useful. By way of introduction, I am providing a couple of examples. If you want to test this out for yourself, the development version of `simstudy` on [GitHub](https://kgoldfeld.github.io/simstudy/dev/index.html){target="_blank"} makes this available.
