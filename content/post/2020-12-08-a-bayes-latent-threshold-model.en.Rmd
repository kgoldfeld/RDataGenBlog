...
The observed data include the continuous outcome $\textbf{y}$ and a continuous antibody measure $\textbf{x}$. There are $M$ possible thresholds provided by the researcher as a vector $\mathbf{c}$.The threshold $c[m], \; m \in 1,\dots,M$ determines the relationship of $x$ to $y$; each candidate threshold is treated as a discrete quantity and a probability $\lambda_m$ is attached to each. Here is the model for the outcome conditional on the distribution parameters as well as the probability of the thresholds:
...
Here is a translation of the model into `Stan`. The data submitted to Stan include the antibody level $x$, the outcome $y$, as well as the candidate thresholds included in the vector $\mathbf{c}$ which has length $M$. In this example, the candidate vector is based on the range of observed antibody levels.
...
To start, equal probability will be assigned to each of the $M$ candidate thresholds, so that will be $1/M$. Since Stan lives in log-probability world, this is translated to $\text{log}(1/M) = \text{-log}(M)$:
...
This next block is really the implementation of the threshold model. $lp$ is a vector of log probabilities, where each element represents the log of each summand in the model specified above. 
...
![](/img/post-bayesthreshold/p3.png){height=80%}

### Model fitting

We use the `rstan` package to sample from the Stan model, passing along the observed antibody data, outcome data, as well as the candidate thresholds:
...
![](/img/post-bayesthreshold/p0.png){height=80%}
...
And finally, here is the posterior distribution of thresholds. The plot is intentionally drawn to highlight the lack of distinction across the candidate thresholds:
...
Here's a little more background on how the posterior probabilities for the threshold were calculated. As a reminder, $c$ is a vector of candidate thresholds of length $M$. We define a quantity $q(c_m | data)$ as
...
To get around this problem, I was used the `mpfr` function in the `Rmfpr` package. Here is a simple example to show how exponentiate a very negative variable $b$. A helper variable $a$ is specified to set the precision, which can then be used to derive the desired result, which is $\text{exp}(b)$.
