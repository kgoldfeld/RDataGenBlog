...
Double-dot notation was [introduced](https://www.rdatagen.net/post/simstudy-just-got-a-little-more-dynamic-version-0-2-0/) in the last version of `simstudy` to allow data definitions to be more dynamic. Previously, the double-dot variable was a scalar value, but with the current version, double-dot notation is also *array-friendly*. 

Here are the necessary packages for this post:
...
The data definition references each cell of the vector:
...
The function [trtAssign](https://kgoldfeld.github.io/simstudy/articles/treat_and_exposure.html){target="_blank"} provides functionality to randomize group assignments using stratification and non-standard ratios (e.g. 2:2:1 for a three arm trial). Now, it is also possible to generate these treatment assignments directly in the `defData` and `genData` process without a separate call to `trtAssign`. We've done this by adding *trtAssign* as a possible distribution. 

In this example, randomization is stratified by *gender* and *age*, and the outcome *y* is effected by both of these factors as well as the treatment assignment variable *rx*.
...
In previous versions, the *categorical* distribution generated a set of integer categories:
...
And to replicate the Example 1 above, here is an alternative way to generate block sizes of 2 and 4 using the *categorical* distribution.
