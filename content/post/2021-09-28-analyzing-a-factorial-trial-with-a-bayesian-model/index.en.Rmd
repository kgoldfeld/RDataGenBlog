...
The `Stan` implementation is shown below in the <a href="#addendum">addendum</a>. The function shown here simply generates the data needed by `Stan`. The most important element is the conversion of the $\tau_{ab}$ design matrix of 0's and 1's to a single vector with values ranging from 1 to 16.
...
The plot of the observed means is consistent with a main effect only for intervention $a$, but no effect for $b$ and no interaction:
...
The posterior distribution for $\sigma_{ab}$ (*sigma_x*) is now very close to zero ...
