...
In this three-arm cluster randomized trial there are three levels of measurement: the patient, the provider, and the clinic. Randomization will be conducted at the provider level, stratified by clinic. The hazard for individual $i$, treated by provider $j$ in clinic $k$, can be modeled as:
...
* $\beta_1$ is the effect of $A$ alone,
...
* $A_{ijk}$ is a treatment indicator for patient $i$ seeing provider $j$ in clinic $k$: $A_{ijk} = 1$ whe provider $j$ has been randomized to treatment $A$, and $0$ otherwise.
* $AB_{ijk}$ is a treatment indicator for patient $i$ seeing provider $j$ in clinic $k$: $AB_{ijk} = 1$ when provider $j$ has been randomized to treatment $A + B$, and 0 otherwise.

#### Data definititions

```{r}
defC <- defData(varname = "g", formula = 0, variance = "..v_clinic")

defP <- 
  defDataAdd(varname = "b", formula = 0, variance = "..v_prov") |>
