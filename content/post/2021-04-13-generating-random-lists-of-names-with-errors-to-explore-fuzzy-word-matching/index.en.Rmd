...
Health data systems are not always perfect. This was painfully obvious when a study I am involved with tried to match a list of nursing home residents taken from one system with a list of results from PCR tests for COVID-19 drawn from another. Name spellings for the same person from the second list were not always consistent across different PCR tests, nor were they always consistent with the cohort we were interested in studying defined by the first list. My research associate, Yifan Xu, and I were asked to help out and see what we could do to help out.

I've never thought about fuzzy word-matching before, so I certainly qualify as a novice. But this didn't stop us from coming up with simple solution to match the names on the two lists based on the `R` function `adist` that I think will allow the research team to finalize a matched list with minimal manual effort. In order to test our proposed approach, we developed a way to generate random lists of names with errors. This post presents both the code for random list generation with errors as well as the simple matching algorithm.

### Distance between strings

Fuzzy word matching can be approached using the concept of *string distance*. Quite simply, this can be measured by counting the number of transformations required to move from the original to the target word. A transformation is one of three moves: (1) a substitution, (2) an insertion, or (3) a deletion. The figure below illustrates the 5 "moves" that are required to get from **CAT** to **KITTEN**: two letter substitutions and three letter insertions.
...
Assessing whether a distance is meaningful or no longer fuzzy certainly depends on the nature of the problem and the length of a strings. The distance from **CAT** to **DOG** is 3 (with 3 substitutions), and so is the distance from **DERE, STEPHEN** to **DEERE, STEVEN** (1 insertion, 1 deletion, and 1 substitution); however, we might be willing to declare the names a match while declining to pair the two different animals.

### Random lists of names with errors

To test out our fuzzy matching process, we need to be able to create a master list of names from which we can create two sub-lists: (1) a cohort list of nursing home residents with correct name spellings, and (2) a list of PCR records that includes multiple records (test results) per individual, with possible inconsistent name spellings across the different tests for a specific person.

#### Generating names

The master list can easily be generated using an the `randomNames` function in the R package [randomNames](https://centerforassessment.github.io/randomNames/){target="_blank"}. A call to this function provides samples of names from a large scale database. (It provides gender and ethnic variation if you need it.)
...
Finally, we end up with two lists - one with just residents only and one with a list of PCR tests. This is mimicking the actual data we might get from our flawed health data systems.
...
We see that we did pretty well, identifying 12 of the 14 resident records in the PCR data. The two we missed were the result of flipped names. We have actually implemented this check as well by reversing the order of the first and last names, and re-calculating the distance for each pair. I've omitted that here just to simplify a bit.
