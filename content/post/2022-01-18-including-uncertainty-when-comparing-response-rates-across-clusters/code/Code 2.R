...
def <- defData(varname = "theta", formula = .80, variance = 30, dist="beta")
def <- defData(def, varname = "n", formula = 100, variance = .3, dist="negBinomial")
def <- defData(def, varname = "y", formula = "theta", variance = "n", dist = "binomial")
...
dp <- data.table(sites, theta = dd$theta, p_est, ci[, .(l95_p, u95_p)])
...
ggplot(data = dp, aes(x = p_est, y = index)) +
  ...
  geom_point(size = .5) +
  geom_point(aes(x = theta), color = "red", size = .5, pch = 4) +
...
ggplot(data = df, aes(dist = theta, y = index)) +
  geom_vline(xintercept = 0.75, color = "white", size = .8) +
  geom_point(aes(x = true), color = "red", size = .5, pch = 4) +
