...
In the data generation process, the treatment effect will be a parameter $\theta$ that will be determined by the levels of the three covariates. In this case, for patients $A=B=C=0$, there will be no treatment effect. However, for patients with $A=1$, there will be a small treatment effect of $2$, and there will be a slightly larger effect of $4$ for patients with $C=1$, and for patients with $A=1 \ \& \ C=1$, there will be a treatment effect of $5$. For patients with $B=1$ there is no treatment effect.
...
In the data generation process, I am assigning group identifiers based on the covariates that will be relevant for the Bayes model (described further below).
...
Inspecting the point estimates (denoted as *p.50* for the treatment effect for each subgroup (and the overall group), we see that they match pretty closely with the effect sizes depicted in the figure of the means by subgroup above. I compare these estimates to the Bayes estimates in a bit.
...
The treatment effect parameter $\theta_j$ can be further parameterized as function of a set of $\tau$'s. (This parameterization was inspired by this [*Jones et al* paper](https://journals.sagepub.com/doi/full/10.1177/1740774510396933){target="_blank"}.) In this parameterization, the treatment effect is a function of the covariates $a$, $b$, and $c$ as well as their interaction:
...
So far, the parameterization for the *pooled* and *unpooled* models are the same. And this is where the models diverge. The idea behind the *pooled* model is that the *main effects* of $a$, $b$, $c$ ($\tau_a$, $\tau_b$, and $\tau_c$, respectively) are drawn from the same distribution centered around $\delta_m$ with a standard deviation $\sigma_m$, both of which will be estimated from the data. This means that the effect of one covariate will inform to some extent the effect of the others. Of course, as the number of observations increases, the extent of pooling will be reduced. The three 2-level interaction effects ($\tau_{ab}$, $\tau_{ac}$ and $\tau_{bc}$) are independent of the main effects, but they share a common distribution to be estimated from the data. (In this case, the $\alpha_j$ intercepts are also pooled, but we could just as easily relax this, as we are not particularly interested in these parameters.)
...
With the exception of $\sigma_0$ and $\sigma_\alpha$, the prior distributions for the model parameters are quite conservative/pessimistic, centered pretty closely around 0. (It would certainly be wise to explore how these prior assumptions impact the findings, but since this is just an illustrative example, I won't dwell too much on these particular assumptions).
...
In the unpooled model, the $\tau$'s (and $\alpha$'s) are not jointly parameterized with a common mean, and prior distributions are more diffuse. The only variance estimation is for $\sigma_0$:
...
The second point is that the point estimates for the linear regression model and the median estimates for the *unpooled* model are quite close, while the *pooled* medians appear to be pulled away. The direction of the shrinkage is not coherent, because there is a mixture of main effects and interaction effects (the $\tau$'s) that are shifting things around. It appears that the effects of the subgroups $a=1$, $b=1$, and $c=1$ are being pulled towards each other, and maybe the same can be said for the group defined by $a=0$, $b=0$, and $c=0$.
