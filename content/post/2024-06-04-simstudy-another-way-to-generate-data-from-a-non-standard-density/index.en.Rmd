...
Well, that was quick. Andrea provided code on Disqus - which for some reason is no longer publishing on my site, and if anyone has thoughts about that issue, feel free to contact me :) - that does exactly what I was trying to do without any pre-plotting data transformations. The trick is to use the *density* stat available in `geom_histogram`, This acutally looks better, because it lines up more precisely with the density curve.
