...
We can look at the density to make sure this is the shape we are interested in drawing our data from:
...
Drawing samples from this distribution is a simple call to `genDataDensity`. The key argument is the data distribution as as represented by the vector of integers:  

```{r, fig.width = 8, fig.height = 4, warning=FALSE}
dx1 <- genDataDensity(10000, dataDist = base_data, varname = "x1")
```

Here's a look at the sampled data and their relationship to the target density:

```{r, warning=FALSE, fig.height = 4, fig.width = 8}
ggplot(data = dx1, aes(x=x1)) +
...
```{r}
dx2 <- genDataDensity(10000, dataDist = base_data[[1]], varname = "x1")
dx3 <- genDataDensity(10000, dataDist = base_data[[2]], varname = "x1")
dx4 <- genDataDensity(10000, dataDist = base_data[[3]], varname = "x1")
```

```{r, fig.width = 10, fig.height = 2.75, warning=FALSE, echo = FALSE}
dx <- rbindlist(list(dx2, dx3, dx4), idcol = TRUE)
...
This is a little off topic, but thought I'd share in case some one, some day might find the code useful to generate the "facet" plot. In the plot with the single distribution, I specified the histogram with this command:

```{r, eval=FALSE}
geom_histogram(aes(y = after_stat(count / sum(count))), ...)
```               

When I tried to apply this to the "facet" plot, the denominator of that plot (`sum(count)`) is not calculated for each subgroup (i.e., data set), but is the total across all data sets. As a result, the data set specific proportions were way too low; we can see that here:
...
I looked around for a way to address this, but couldn't find anything that obviously addressed this shortcoming (though I am convinced it must be possible). I considered using `ggarrange`or something similar, but was not satisfied with the results. Instead, I though it would be faster just to calculate the proportions myself. This is the process I used:

First, I created a data set with the bins (using a bin size of 1):
