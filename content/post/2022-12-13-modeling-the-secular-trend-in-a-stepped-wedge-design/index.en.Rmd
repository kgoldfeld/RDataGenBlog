...
```{r}
gamfit <- gamm4(y ~ A + s(k, site, bs = "fs", k = 5), data = dd)  
...
fitlme <- lmer(y ~ A + ( ns(normk, knots = knots) - 1 | site)  , data = dd) # 4
summary(fitlme)
```

```{r, echo = FALSE, fig.width = 10, fig.height = 4}
ddpred <- dd[, .SD[1,] , keyby = .(site, k)][, .(site, k, normk, A)]
ddpred$y <- predict(fitlme, ddpred)
...
fitlme2 <- lmer(y ~ A + factor(k) - 1 + (1|site)  , data = dd)
summary(fitlme2)
```
