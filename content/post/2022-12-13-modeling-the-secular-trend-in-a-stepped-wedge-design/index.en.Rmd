...
I am fitting three models to this simulated data set: (1) specific (fixed) time period effects, (2), a mixed effects model with a random cubic spline for the period effect for each site, and (3) a generalized additive model with a site-specific smooth function for time. For each one, I will plot the predicted values over the actual data points.

Since there is only so much one can glean from a single data set, I've also conducted a experiment using 5000 replicated data sets to see how each model really performs with respect to the estimation of the treatment effect. I'll report the bias of the estimate, the root mean squared error (RMSE), the true standard error of the estimate, the average estimated standard error, and the coverage rate (i.e. proportion of 95% confidence intervals that include the true value 5).
...
```{r}
gamfit <- gam(y ~ A + s(k, site, bs = "fs", k = 10), data = dd)
summary(gamfit)
```
...
```{r, echo = FALSE}
load("replications/res10.8.Rdata")
```
...
dres[, mean(mapply("coverage", 5, est.lmek, se.lmek))]
dres[, mean(mapply("coverage", 5, est.lmes, se.lmes))]
dres[, mean(mapply("coverage", 5, est.gam, se.gam))]
