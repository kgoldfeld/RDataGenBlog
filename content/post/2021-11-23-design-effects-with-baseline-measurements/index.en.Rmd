...
This post focuses on work already done to derive *design effects* for parallel cluster randomized trials (CRTs) that collect baseline measurements; we will get to stepped wedge designs in future posts. I described the [design effect](https://www.rdatagen.net/post/what-exactly-is-the-design-effect/){target="_blank"} pretty generally in an earlier post, but a [paper](https://onlinelibrary.wiley.com/doi/full/10.1002/sim.5352){target="_blank"} by *Teerenstra et al*, titled "A simple sample size formula for analysis of covariance in cluster randomized trials" provides a great foundation to understand how baseline measurements can impact sample sizes in clustered designs.
