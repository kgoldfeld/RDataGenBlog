...
The goal is to reduce the odds of having a bad outcome, so a successful therapy is one where $OR \lt 1$. In a Bayesian context, we estimate the posterior probability distribution of the $OR$ (based on prior assumptions about the distribution before we collected any data). We will recommend the therapy in the case that most of the probability distribution lies to the left of 1; in particular we will claim success only when $P(OR \lt 1) > 0.95$. For example, in the figure the posterior distribution on top would lead us to consider the therapy successful since 95% of the density falls below 1, whereas the distribution on the bottom would not:
...
ggplot(data = sumdd, aes(x = factor(y, labels = 0:10), y = cumprop)) +
  ...
  scale_color_manual(values = c("#ebd687", "#eb879c"), 
...
where $T_i$ is the treatment indicator for patient $i$, and $T_i = 1$ when patient $i$ receives CP. I've included the Stan code for the model in the the first <a href="#addendumA">addendum</a>.
...
A visual showing the bottom 95\% portion of the density in blue makes it clear that the threshold has not been met:
...
The next step is to put all of these elements in a single function that we can call repeatedly. We can estimate the predictive probability of success by calculating the proportion of iterations that resulted in a success. If we iterate 1000 times, this means we need to fit 1000 Bayesian models, and this could be prohibitive with standard computing resources; a high performance computing cluster may be necessary. (I touched on this [earlier](https://www.rdatagen.net/post/a-frequentist-bayesian-exploring-frequentist-properties-of-bayesian-models/){target="_blank"} when I describe exploring the characteristic properties of Bayesian models.) I have provided the code below in the second  <a href="#addendumB">addendum</a> in case any readers are interested in trying to implement on an HPC.
